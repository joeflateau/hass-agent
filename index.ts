...
const envSchema = z.object({
  ...
  DEVICE_NAME: z.string().default(() => hostname()),
  UPDATE_INTERVAL: z
    .string()
    .regex(/^\d+$/)
    .transform(Number)
...
class MacOSPowerAgent {
  ...
  constructor() {
    ...
    this.client = mqtt.connect(config.MQTT_BROKER, {
  ...
  private publishDiscoveryConfigs(): void {
    ...
    const batteryLevelConfig = {
      name: `${config.DEVICE_NAME} Battery Level`,
    ...
    const batteryChargingConfig = {
      name: `${config.DEVICE_NAME} Battery Charging`,
    ...
    // AC Power Sensor
    const acPowerConfig = {
      name: `${config.DEVICE_NAME} AC Power`,
    ...
    const timeRemainingConfig = {
      name: `${config.DEVICE_NAME} Battery Time Remaining`,
  ...
  private startMonitoring(): void {
    ...
    console.log(`Started monitoring with ${config.UPDATE_INTERVAL}ms interval`);
  }
...
const agent = new MacOSPowerAgent();
